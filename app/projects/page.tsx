...
export const metadata: Metadata = {
  title: "نمونه‌کارها",
  description: "پروژه‌های موکیوب بر اساس خدمت و مشتری",
};

type PageProps = {
  searchParams: Promise<{ service?: string; customer?: string }>;
};

export default async function ProjectsPage({ searchParams }: PageProps) {
  const params = await searchParams;
  const [projects, services, customers] = await Promise.all([
      ...
      <Container>
        <Reveal>
          ...
          <p className="mt-16 mb-32 max-w-2xl text-body-lg tracking-body-lg text-surface-50">
            فیلتر بر اساس خدمت یا مشتری — هر پروژه به هر دو وصل است.
          </p>
        </Reveal>
