  ...
  <div>
    <b-container dir="rtl">
      ...
      <b-row>
        <b-col>
          <div class="titleBox">
            <h1 class="title">{{ questions[index].question }}</h1>
          </div>
        </b-col>
      </b-row>
      <b-row>
        <b-col>
          <div class="illustrationBox">
            <img :src="questions[index].illustration" class="illustration">
          </div>
        </b-col>
      </b-row>
      <b-form-row>
        <b-col>
          <b-button block variant="light" class="mainBut" size="lg" @click="firstAnswer">{{ questions[index].answer1 }}</b-button>
        </b-col>
        <b-col>
          <b-button block variant="light" class="mainBut" size="lg" @click="secondAnswer">{{ questions[index].answer2 }}</b-button>
        </b-col>
      </b-form-row>
      <b-form-row>
        <b-col>
          <b-progress :value="index + 1" max="15" height="2px" class="progressBar"></b-progress>
        </b-col>
      </b-form-row>
    </b-container>
  </div>
    ...
    firstAnswer () {
      if (this.index < 14) {
        ...
        if (this.artist.includes(this.score)) {
          this.$router.push({name: 'types-artist', params: { id: 1 }})
        } else if (this.thinker.includes(this.score)) {
          this.$router.push({name: 'types-thinker', params: { id: 1 }})
        } else if (this.adventurer.includes(this.score)) {
          this.$router.push({name: 'types-adventurer', params: { id: 1 }})
        } else if (this.maker.includes(this.score)) {
          this.$router.push({name: 'types-maker', params: { id: 1 }})
        } else if (this.producer.includes(this.score)) {
          this.$router.push({name: 'types-producer', params: { id: 1 }})
        } else if (this.dreamer.includes(this.score)) {
          this.$router.push({name: 'types-dreamer', params: { id: 1 }})
        } else if (this.innovator.includes(this.score)) {
          this.$router.push({name: 'types-innovator', params: { id: 1 }})
        } else {
          this.$router.push({name: 'types-visionary', params: { id: 1 }})
        }
      }
    },
    secondAnswer () {
